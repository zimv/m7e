...
const Tabs = ({ images, onSelect }) => {
  ...
  const height = 400;
  const getPosX = (i) =>
    width / 2 - (images.length * (circle * 2 + gap) - gap) / 2 + i * (circle * 2 + gap);
...
const ImageGallery = () => {
  ...
  return (
    <div className={classnames(styles.gallery, 'flex items-center')}>
      <div className={classnames(styles.container, styles.shadow, 'flex-1')}>
        {images.map((image, i) => {
          return (
            <div
              key={image.url}
        ...
        <button
          className={classnames(styles.button, styles.next, styles.shadow, 'flex-1')}
          onClick={next}
        >
      ...
      <div className="flex flex-1 flex-col">
        <div className="p-20">
          <h1 className="text-purple-500 text-6xl font-semibold">{active.title}</h1>
          <p className="text-gray-050 text-xl font-light mt-10">{active.description}</p>
        </div>
      </div>
    </div>
  );
